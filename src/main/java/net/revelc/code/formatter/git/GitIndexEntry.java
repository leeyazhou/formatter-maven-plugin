...
class GitIndexEntry {
    ...
    private static class EntryFormatter extends PathEdit {
        ...
        private void doFormat(DirCacheEntry dirCacheEntry, Formatter formatter) {
            ...
            try (TemporaryFile temporaryFormattedFile = TemporaryFile.create(log,
                    dirCacheEntry.getPathString() + ".formatted")) {
        ...
        private LineRanges computeLineRanges(DirCacheEntry dirCacheEntry) {
            try (Git git = new Git(repository)) {
                ...
                try {
                    partiallyStaged = !git.status().addPath(dirCacheEntry.getPathString()).call().getModified()
                            .isEmpty();
                ...
                try {
                    return git.diff().setPathFilter(PathFilter.create(dirCacheEntry.getPathString())).setCached(true)
                            .call().stream().map(this::computeLineRanges).reduce(LineRanges::concat)
