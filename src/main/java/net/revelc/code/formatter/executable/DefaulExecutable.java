...
class DefaulExecutable implements Executable {
    ...
    @Override
    public Executable truncateWithTemplate(Supplier<InputStream> template, String sourceEncoding, Object... values)
            throws IOException {
    ...
    public Executable appendCommandCall(String commandCall) throws IOException {
        ...
        boolean callExists = Files.readAllLines(file).stream().anyMatch(s -> s.contains(unixCommandCall));
        if (callExists) {
    ...
    public Executable removeCommandCall(String commandCall) {
        String unixCommandCall = unixifyPath(commandCall, true);
        try {
            List<String> linesToKeep = Files.readAllLines(file).stream().filter(line -> !unixCommandCall.equals(line))
